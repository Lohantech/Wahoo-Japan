...
function filterTZ(q) {
  ...
  const filtered = q
    ? ALL_TZ.filter(z => z.toLowerCase().includes(q))
    : ALL_TZ.filter((_, i) => i < 50);
...
function updateTZLabel() {
  ...
  const label = st.timezone.replace(/_/g, ' ') + (off ? ' (' + off + ')' : '');
  document.getElementById('tz-cur-label').textContent = label;
...
function applyLang() {
  ...
  const lbl = document.getElementById('s-btn-label');
  if (lbl) lbl.textContent = L.searchBtn;
  const tzSrch = document.getElementById('tz-search');
  if (tzSrch) tzSrch.placeholder = L.tzSearch;
  const tzReset = document.getElementById('tz-reset-btn');
  if (tzReset) tzReset.textContent = L.tzReset;
}
...
function saveCustomEng() {
  ...
  if (cEl) { cEl.classList.add('sel'); const em = cEl.querySelector('em'); if (em) em.textContent = name; }
...
function doSearch() {
  ...
  if (q) window.location.href = buildURL(q);
}
...
function openSlotPicker(i) {
  _pendingSlot = i;
  const f = document.getElementById('slot-file');
  f.value = ''; f.click();
}
...
function activateSlot(i) {
  ...
  document.body.classList.add('has-wallpaper');
  renderCustomSlots(); save();
}

function deleteSlot(evt, i) {
  evt.stopPropagation();
  if (st.wallpaper === `u${i}`) setWP('none', document.querySelector('.wp-t[data-wp="none"]'));
...
function exportSave() {
  const payload = {
    ...
    mode:st.mode, overlay:st.overlay,
...
function applyState(d) {
  ...
  if (isSlot) {
    ...
    setWP(wp, document.querySelector(`.wp-t[data-wp="${wp}"]`));
  }
  if (d.mode)    st.mode    = d.mode;
  if (d.overlay !== undefined) st.overlay = !!d.overlay;
  applyMode(); applyLang(); renderCustomSlots();
  if (Array.isArray(d.shortcuts)) { st.shortcuts = d.shortcuts; saveShortcuts(); }
